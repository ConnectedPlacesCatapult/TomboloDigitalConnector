...
shp2pgsql -W LATIN1 /tmp/la/data/CTYUA_DEC_2012_EW_BFE.shp la | psql -d tombolo
psql -d tombolo -c "insert into subject_object(subject_type_label, label, name, shape) select 'localAuthority', ctyua12cd, ctyua12nm, ST_Transform(ST_SETSRID(geom, 27700),4326) from la"
