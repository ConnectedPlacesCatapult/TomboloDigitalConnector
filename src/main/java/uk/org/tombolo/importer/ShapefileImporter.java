...
public abstract class ShapefileImporter extends AbstractImporter implements Importer{
    private static Logger log = LoggerFactory.getLogger(ShapefileImporter.class);

    protected FeatureReader getFeatureReader(ShapefileDataStore store, int index) throws IOException {
        DefaultQuery query = new DefaultQuery(store.getTypeNames()[index]);
        return store.getFeatureReader(query, Transaction.AUTO_COMMIT);
    }

    protected MathTransform makeCrsTransform() throws FactoryException {
    ...
    protected List<Subject> convertFeaturesToSubjects(FeatureReader<SimpleFeatureType, SimpleFeature> featureReader, SubjectType subjectType) throws FactoryException, IOException, TransformException {
        MathTransform crsTransform = makeCrsTransform();

        List<Subject> subjects = new ArrayList<>();
        while (featureReader.hasNext()) {
            SimpleFeature feature = featureReader.next();
            String label = getFeatureSubjectLabel(feature, subjectType);
            String name = getFeatureSubjectName(feature, subjectType);
            ...
            try {
                ...
                log.warn("Rejecting {}. You will see this if you have assertions enabled (e.g. " +
                        "you run with `-ea`) as GeoTools runs asserts. See source of ShapefileImporter for details on this. " +
